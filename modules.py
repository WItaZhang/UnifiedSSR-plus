...
class CausalAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, is_target_attn=1):
        # 创建一个上三角掩码，确保每个位置只能attend到之前的位置
        seq_len_q = query.size(1)
        seq_len_k = key.size(1)
        # print(is_target_attn)
        
        if is_target_attn==1:
            causal_mask = torch.triu(torch.ones((seq_len_q, seq_len_k)), diagonal=1).bool()
            if mask is not None:
                # 结合输入mask和因果mask
                mask = mask.unsqueeze(1) & ~causal_mask.to(mask.device)
            else:
                mask = ~causal_mask.to(query.device)
        else:
            if mask != None:
                # Same mask applied to all h heads.
                mask = mask.unsqueeze(1)
...
class SequencePartition(nn.Module):
    ...
    def forward(self, x, last_idx):
        ...
        # 将 last_idx 扩展为 [batch_size, 1, 1]
        last_idx = last_idx.unsqueeze(-1).unsqueeze(-1)  # 增加两个维度
        sub_seq_wins = sub_seq_wins * last_idx
        return sub_seq_wins
...
class SiameseEncoder(nn.Module):
    ...
    def forward(self, src1, src2, src3, mask, is_target_attn=0):
        for layer in self.layers:
            x = layer(src1, src2, src3, mask, is_target_attn)
        return self.norm(x)
...
class SiameseEncoderLayer(nn.Module):
    def __init__(self, emb_size, hid_size, num_head, dropout):
        ...
        self.self_attn = CausalAttention(num_head, emb_size)
        self.cross_attn = CausalAttention(num_head, emb_size)
        self.target_attn = CausalAttention(num_head, emb_size)
        self.feed_forward = PositionwiseFeedForward(emb_size, hid_size, dropout)
        self.sublayer = clones(SublayerConnection(emb_size, dropout), 4)

    def forward(self, src1, src2, src3, src_mask, is_target_attn=0):
        ...
        src3 = self.sublayer[2](src3, lambda src3: self.target_attn(src3, src1, src1, src_mask, is_target_attn))
        return self.sublayer[3](src3, self.feed_forward)
