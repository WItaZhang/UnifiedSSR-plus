...
def run_epoch(args, model, data_loader, optimizer, epoch_idx, batch_num, device):
	...
	for idx, (cur_task, batch_data) in enumerate(data_loader):
		...
		if cur_task == 'recommendation':
			if args.corr_factor > 0:
				sub_seq_wins = model.get_sub_seq_wins(out,batch_data['pid_last_idx'])
			...
			p_out, q_out, q_last = out
			p_out = p_out[:, :, :]
			q_out = q_out[:, :, :]
			q_last = q_last[:, :, :]
            # seq_len = q_out.size(1)
            # q_last = q_last.expand(-1,seq_len,-1)

            # 确保 q_last_expanded 的形状与 q_out 相同
			assert q_last.shape == q_out.shape
			mask = batch_data['pids_mask'][:, :, :]

			# 如果相关性因子大于0，则计算相关性损失
			if args.corr_factor > 0:
				p_sub_seq_wins = model.get_sub_seq_wins(p_out, batch_data['pid_last_idx'])
				q_sub_seq_wins = model.get_sub_seq_wins(q_out, batch_data['pid_last_idx'])
			...
			out_loss = model.loss(p_out.reshape(-1, p_out.size(-1)),
									...
									q_last.reshape(-1, q_last.size(-1)),
									mask.reshape(-1),
									batch_data['pids_tgt'].view(-1),
									batch_data['pids_neg'].view(-1, args.train_num_neg))
	...
	# if (epoch_idx>56 and epoch_idx<64):
	# 	save_model(model, args.save_dir, epoch_idx)
...
if __name__ == '__main__':
	...
	# args.tasks = ['recommendation']
	args.tasks = ['search']
	# args.sub_seq_num = 2
	# args.enc_num_layer = 2
