        ...
        for idx, (cur_task, batch_data) in enumerate(data_loader):
            ...
            if cur_task == 'recommendation':
                if args.corr_factor > 0:
                    sub_seq_wins = trained_model.get_sub_seq_wins(out,batch_data['pid_last_idx'])
                ...
                if args.corr_factor > 0:
                    ...
                    p_sub_seq_wins = trained_model.get_sub_seq_wins(p_out, batch_data['pid_last_idx'])
                    q_sub_seq_wins = trained_model.get_sub_seq_wins(q_out, batch_data['pid_last_idx'])
                    p_out, q_out, _ = trained_model.inter_corr_loss(p_out, p_sub_seq_wins, q_out, q_sub_seq_wins, mask)

                if args.test_neg:  # rank candidates
                    out = trained_model.next_product_search(p_out, q_out, q_last, batch_data['pid_last_idx'],
                                                            batch_data['pid_pred'])
                    tgt = torch.zeros((out.shape[0], 1)).long()
                else:  # rank all products
                    out = trained_model.next_product_search(p_out, q_out, q_last, batch_data['pid_last_idx'])
